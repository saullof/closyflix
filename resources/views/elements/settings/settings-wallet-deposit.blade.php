...
@if (Session::has('noxpay_payment_data') && Session::get('noxpay_payment_data')['user_id'] == Auth::user()->id)
    ...
    <div class="mt-4 text-center">
        @if (!empty($noxpayData['noxpay_payment_url']))
            <div class="noxpay-checkout-frame-wrapper mx-auto" style="max-width: 420px;">
                <iframe
            ...
            <p class="small text-muted mt-3 mb-1">{{ __('Complete the checkout above to finalize your deposit.') }}</p>
            <p class="small mb-2">
            ...
            @endif
        @elseif (!empty($noxpayData['noxpay_qr_code']))
