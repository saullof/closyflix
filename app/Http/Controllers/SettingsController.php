...
class SettingsController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        switch ($request->route('type')) {
            case 'pixel':
                ...
                $pixels = UserPixel::where("user_id", $userID)->get()->all();
                if(count($pixels) != 4){
                    $pixels_o = ["meta", "google", "tiktok", "twitter"];
                    foreach($pixels_o as $pixel_type){
                        $p = UserPixel::where("user_id", $userID)->where('type', $pixel_type)->count();
                        if($p == 0){
                            UserPixel::create(array(
                                "user_id" => $userID,
                                "type" => $pixel_type,
                                "head" => "",
                                "body" => "",
                ...
                $pixel_data = array();
                $data_order = ["meta-head", "meta-body", "google-head", "google-body","tiktok-head", "tiktok-body", "twitter-head", "twitter-body"];
                foreach($data_order as $pixel_name){
                    array_push($pixel_data, $pixel_obj[$pixel_name]);
                }

                $data['pixel_data'] = $pixel_obj;
                break;
            case 'wallet':
                JavaScript::put([
                    'stripeConfig' => [
    ...
    public function savePixel(Request $request)
    {
        ...
        foreach($pixels_o as $pixel_type){
            $userPixel = UserPixel::where('user_id', Auth::user()->id)->where('type', $pixel_type)->first();
            $head = $request->input($pixel_type."-head");
            $body = $request->input($pixel_type."-body");

            $data = [
                'head' => $head == null ? "" : $head,
                'body' => $body == null ? "" : $body,
            ];
            $userPixel->update($data);
        }
