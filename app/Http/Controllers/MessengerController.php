...
class MessengerController extends Controller
{
    ...
    public function fetchMessages(Request $request)
    {
        ...
        $conversation = UserMessage::with(['sender', 'receiver', 'attachments'])
            ...
            ->leftJoin('transactions', function ($join) {
                $join->on('transactions.user_message_id', '=', 'user_messages.id')
                    ->where('transactions.type', '=', 'message-unlock')  // Ajuste conforme necessário
            ...
            ->map(function ($message) use ($senderID) {
                ...
                $message = self::cleanUpMessageData($message, true);
                return $message;
            });
    ...
    public function sendMessage(Request $request)
    {
        ...
        foreach($receiverIDs as $receiverID){
            ...
            $isPaidMessage = $request->get('price') > 0; // Verificação se a mensagem é paga
        ...
        return response()->json([
            ...
            'isPaidMessage' => $isPaidMessage // Adiciona se a mensagem é paga ou não na resposta
        ]);
    }
    ...
    public static function cleanUpMessageData($message,$fetch = false){
        ...
        if(!$fetch && $message->hasUserUnlockedMessage == false && ($message->price && $message->price > 0) && $message->sender->id !== Auth::user()->id){
    ...
    public function markAllAsRead()
    {
        ...
        // Atualiza todas as mensagens do usuário para 'lidas'
