...
class PaymentsController extends Controller
{
    ...
    public function initiatePayment(CreateTransactionRequest $request)
    {
        ...
        try {
            ...
            $transaction['amount'] = $request->get('amount'); // Valor inicial
            ...
            if ($transaction['payment_provider'] != Transaction::STRIPE_PROVIDER) {
                ...
                if ($coupon) {
                    // Aqui você pode buscar as informações do cupom e calcular o desconto
                    $couponData = $this->paymentHandler->getCouponDetails($coupon); // Método que retorna os detalhes do cupom
                    if ($couponData && isset($couponData['discount'])) {
                        $discount = $couponData['discount'];
                        // Verifica o tipo do desconto e aplica ao valor
            ...
            // Garante que o valor não seja menor que R$5
     ...
     * @param $request
     */
    public function updateUserBillingDetails(Request $request)
    {
        ...
        $phone      = $request->get('phone'); // ← novo
    
        // update user billing details if they changed
        if ($firstName || $lastName || $billingAddress || $phone) {
            $user = Auth::user();
            if ($user) {
                $updateData = [];
                ...
                if ($billingAddress && $billingAddress !== $user->billing_address) {
                ...
                if ($phone && $phone !== $user->phone) {      // ← tratamento
