...
Tinify::setKey("kK8PlN9VCbrv0y7y9bBtW9CKjK80pYYd"); // Substitua "YOUR_API_KEY" pela sua chave da API
...
public static function createAttachment($file, $directory, $generateThumbnail)
{
    ...
    if (self::getAttachmentType($fileExtension) == 'image') {
        // Otimizar a imagem usando Tinify
        try {
            $source = \Tinify\fromBuffer($fileContent);
            $optimizedContent = $source->toBuffer();
        } catch (\Tinify\Exception $e) {
            // Handle Tinify API errors
            throw new \Exception("Error optimizing image: " . $e->getMessage());
        }

        $jpgImage = Image::make($optimizedContent);
        ...
        if ($fileExtension == 'gif') {
            ...
            $jpgImage->encode('jpg', 60);
            $fileContent = $jpgImage->getEncoded();
            $fileExtension = 'jpg';
            $filePath = $directory . '/' . $fileId . '.' . $fileExtension;
    ...
    if ($generateThumbnail && self::getAttachmentType($fileExtension) === 'image') {
        ...
        // Handle audio and documents (unchanged)
