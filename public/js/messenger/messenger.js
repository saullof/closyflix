...
var messenger = {
    ...
    init: function(userId) {
        this.state.userId = userId; // Define o ID do usuário dinamicamente
    },
    /**
    ...
    reloadContactsList: function () {
        ...
        console.log("userId:", userId); // Verifique o ID do usuário
        
        if (this.state.activeFilter === 'unread') {
            // Filtra contatos que possuem mensagens não lidas pelo usuário autenticado
            filteredContacts = filteredContacts.filter(contact => {
                console.log(`Contact ID: ${contact.contactID}, isSeen: ${contact.isSeen}, Receiver ID: ${contact.receiverID}`);
                // Verifica se o contato é marcado como não lido e se a mensagem foi recebida pelo usuário
                return contact.isSeen === 0 && contact.receiverID == userId;
            });
        }
    
        let contactsHtml = '';
        $.each(filteredContacts, function(key, value) {
        ...
        $('.contact-' + this.state.activeConversationUserID).addClass('contact-active');
    },    
    
    
    /**
     * Filters contacts based on selected filter type
     */
    filterContacts: function (filterType) {
        this.state.activeFilter = filterType;
        this.reloadContactsList();
    },

    /**
     * Marks all contacts as read
     */
    markAllAsRead: function () {
        let userId = this.state.userId;
        $.each(this.state.contacts, function (key, contact) {
            if (contact.receiver_id === userId) {
                contact.isSeen = 1; // Marca todos como lidos localmente
            }
        });
        ...
        // Envia requisição ao servidor para marcar todos como lidos
        $.ajax({
            ...
            success: function (response) {
                console.log('Todas as conversas foram marcadas como lidas.');
        ...
        case 'jpeg':
            attachmentsHtml = `
                    <a href="${file.path}" rel="mswp" title="">
                        <img src="${file.thumbnail}" class="mr-2 message-img mt-2">
                    </a>`;
            break;
        default:
...
$(document).ready(function () {
    // Inicializa o messenger com o ID do usuário autenticado
    var userId = $('meta[name="user-id"]').attr('content');
    messenger.init(userId);

    $('#filter-all').on('click', function () {
        messenger.filterContacts('all');
    });

    $('#filter-unread').on('click', function () {
        messenger.filterContacts('unread');
    });
