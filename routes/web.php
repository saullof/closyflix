...
Route::post('/mark-all-as-read', [MessengerController::class, 'markAllAsRead'])->name('messenger.markAllAsRead');
...
Route::group(['middleware' => ['auth','verified','2fa']], function () {
    ...
    Route::group(['prefix' => 'my', 'as' => 'my.'], function () {
        ...
        Route::post('/settings/pixel/save', ['uses' => 'SettingsController@savePixel', 'as'   => 'settings.pixel.save']);
    ...
    Route::group(['prefix' => 'coupons', 'as' => 'coupons.'], function () {
        ...
        Route::post('/validate', ['uses' => 'CouponController@validateCoupon', 'as' => 'validate']);
        Route::get('/stats/{id}', ['uses' => 'CouponController@getStats', 'as' => 'stats']);
        ...
        Route::post('/apply', ['uses' => 'CouponController@apply', 'as' => 'apply']);
        Route::post('/remove', ['uses' => 'CouponController@remove', 'as' => 'remove']);
    });
    ...
    Route::get('/overview', [SubscriptionsController::class, 'getOverviewData'])->name('overview');
...
// Rota de checkout (deve vir antes de outras rotas com {username} para evitar conflito)
Route::get('/{username}/checkout/{coupon_code?}', 'CheckoutController@index')
    ->name('profile.checkout')
    ->where('username', '^(?!posts|streams|checkout).+$')
    ->where('coupon_code', '[A-Z0-9-]+');

Route::get('/{username}/checkout/{coupon_code?}', 'CheckoutController@index')->name('checkout');
